...
public sealed class DocumentServiceTests
{
    ...
    {
        var applicationId = Guid.NewGuid();
        var service = CreateService(applicationId, out _, out _, new BlockingScanner());
    ...
    {
        var applicationId = Guid.NewGuid();
        var service = CreateService(applicationId, out _, out var storage, new BlockingScanner());
        await using var content = new MemoryStream("%PDF-1.7"u8.ToArray());
    ...
    public async Task RegisterAsync_rejects_invalid_sha256()
    {
        ...
        var exception = await Assert.ThrowsAsync<ArgumentException>(() =>
            service.RegisterAsync(applicationId, DocumentType.Passport, "passport.pdf", "application/pdf", 100, "not-a-sha", $"applications/{applicationId:D}/documents/passport/passport.pdf", CancellationToken.None));
    ...
    public async Task RegisterAsync_rejects_storage_key_for_another_application()
    {
        ...
        var exception = await Assert.ThrowsAsync<ArgumentException>(() =>
            service.RegisterAsync(applicationId, DocumentType.Passport, "passport.pdf", "application/pdf", 100, new string('a', 64), $"applications/{Guid.NewGuid():D}/documents/passport/passport.pdf", CancellationToken.None));
