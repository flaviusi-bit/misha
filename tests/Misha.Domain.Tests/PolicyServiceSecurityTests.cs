...
public sealed class PolicyServiceSecurityTests
{
    ...
    public async Task Passport_provider_exception_details_are_not_exposed_by_policy_evaluation()
    {
        ...
        Assert.DoesNotContain("secret provider token", string.Join(" ", result.Reasons), StringComparison.Ordinal);
        Assert.DoesNotContain("internal database connection details", string.Join(" ", result.Reasons), StringComparison.Ordinal);
    }
