...
public sealed class EfApplicationLifecycleAuditRepository(MishaDbContext db) : IApplicationLifecycleAuditRepository
{
    ...
    public async Task<IReadOnlyList<ApplicationLifecycleAudit>> GetByApplicationAsync(Guid applicationId, CancellationToken cancellationToken) =>
        await db.ApplicationLifecycleAudits.AsNoTracking().Where(x => x.ApplicationId == applicationId).OrderBy(x => x.OccurredAtUtc).ToListAsync(cancellationToken);
}
