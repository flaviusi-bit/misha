...
public sealed class ApplicationService(IApplicationRepository repository,IApplicationLifecycleAuditRepository lifecycleAudits,IOutboxWriter outbox)
{
    public Task<Guid> CreateAsync(string applicantReference, CancellationToken cancellationToken) => CreateAsync(applicantReference, null, "legacy", cancellationToken);
    public async Task<Guid> CreateAsync(string applicantReference,string? idempotencyKey,string tenantId,CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(idempotencyKey)) { var existing=await repository.GetByIdempotencyKeyAsync(idempotencyKey.Trim(),cancellationToken); if(existing is not null){if(!string.Equals(existing.ApplicantReference,applicantReference.Trim(),StringComparison.Ordinal)||!string.Equals(existing.TenantId,normalizedTenant,StringComparison.Ordinal)) throw new InvalidOperationException("The idempotency key has already been used for a different application request."); return existing.Id;} }
        var applicant=await repository.GetOrCreateApplicantAsync(applicantReference,normalizedTenant,cancellationToken);
        var application=Misha.Domain.Applications.Application.Create(applicant.Id,applicant.ExternalReference,idempotencyKey,normalizedTenant);
        var persisted=await repository.AddOrGetExistingAsync(application,cancellationToken);
        if(!string.Equals(persisted.ApplicantReference,applicantReference.Trim(),StringComparison.Ordinal)||!string.Equals(persisted.TenantId,normalizedTenant,StringComparison.Ordinal)) throw new InvalidOperationException("The idempotency key has already been used for a different application request.");
        return persisted.Id;
    }
