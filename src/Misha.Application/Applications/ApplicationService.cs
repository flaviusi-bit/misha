    ...
    IOutboxWriter outbox)
{
    public async Task<Guid> CreateAsync(string applicantReference, string? idempotencyKey, string tenantId, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(idempotencyKey))
        {
            ...
            {
                if (!string.Equals(existing.ApplicantReference, applicantReference.Trim(), StringComparison.Ordinal) || !string.Equals(existing.TenantId, tenantId.Trim(), StringComparison.Ordinal))
        ...
        var application = Misha.Domain.Applications.Application.Create(applicant.Id, applicant.ExternalReference, idempotencyKey, tenantId);
        var persisted = await repository.AddOrGetExistingAsync(application, cancellationToken);
        if (!string.Equals(persisted.ApplicantReference, applicantReference.Trim(), StringComparison.Ordinal) || !string.Equals(persisted.TenantId, tenantId.Trim(), StringComparison.Ordinal))
            throw new InvalidOperationException("The idempotency key has already been used for a different application request.");
        return persisted.Id;
    }

    public Task<Guid> CreateAsync(string applicantReference, CancellationToken cancellationToken) => throw new InvalidOperationException("Tenant-aware application creation is required.");
    ...
    private async Task TransitionAsync(Guid id, string actorReference, string? reason, Action<Misha.Domain.Applications.Application> transition, CancellationToken cancellationToken)
    {
        ...
        var audit = ApplicationLifecycleAudit.Create(application.Id, fromStatus, application.Status, actorReference, reason);
        await lifecycleAudits.AddAsync(audit, cancellationToken);
        var eventId = Guid.NewGuid();
        var occurredAtUtc = audit.OccurredAtUtc;
        var lifecycleEvent = new ApplicationLifecycleChanged(eventId, application.Id, fromStatus.ToString(), application.Status.ToString(), reason, actorReference, occurredAtUtc);
        await outbox.AddAsync(eventId, "application.lifecycle.changed.v1", application.Id, JsonSerializer.Serialize(lifecycleEvent), occurredAtUtc, cancellationToken);
