    ...
    IContentSafetyScanner contentSafetyScanner)
{
    ...
    public async Task<DocumentArtifact> UploadAsync(Guid applicationId, DocumentType documentType, string fileName, string contentType, Stream content, CancellationToken cancellationToken)
    {
        ...
        if (!safety.Allowed) throw new ArgumentException(safety.Reason ?? "Document content was rejected by the safety validation boundary.", nameof(content));
    ...
    public Task<IReadOnlyList<DocumentArtifact>> GetAsync(Guid applicationId, CancellationToken cancellationToken) => documents.GetByApplicationAsync(applicationId, cancellationToken);
    ...
    {
        _ = await applications.GetAsync(applicationId, cancellationToken) ?? throw new KeyNotFoundException($"Application '{applicationId}' was not found.");
        var document = (await documents.GetByApplicationAsync(applicationId, cancellationToken)).SingleOrDefault(x => x.Id == documentId) ?? throw new KeyNotFoundException($"Document '{documentId}' was not found for application '{applicationId}'.");
    ...
    {
        _ = await applications.GetAsync(applicationId, cancellationToken) ?? throw new KeyNotFoundException($"Application '{applicationId}' was not found.");
        var document = (await documents.GetByApplicationAsync(applicationId, cancellationToken)).SingleOrDefault(x => x.Id == documentId);
