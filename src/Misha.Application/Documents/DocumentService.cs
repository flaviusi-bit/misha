    ...
    IContentSafetyScanner contentSafetyScanner)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        buffer.Position = 0;
        var safety = await contentSafetyScanner.ScanAsync(
            buffer,
            fileName.Trim(),
            contentType.Trim(),
            cancellationToken);
        if (!safety.Allowed)
            throw new ArgumentException(
                safety.Reason ?? "Document content was rejected by the safety validation boundary.",
                nameof(content));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!IsValidSha256(sha256))
            throw new ArgumentException(
                "Document SHA-256 must be a 64-character hexadecimal value.",
                nameof(sha256));
        ...
        {
            throw new ArgumentException(
                "Document storage key must belong to the target application.",
                nameof(storageKey));
        }
    ...
    public async Task<IReadOnlyList<DocumentArtifact>> GetAsync(
        Guid applicationId,
        CancellationToken cancellationToken)
    {
        _ = await applications.GetAsync(applicationId, cancellationToken)
        ...
        return await documents.GetByApplicationAsync(applicationId, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return (
            storageKey,
            storage.CreatePreSignedUploadUrl(storageKey, contentType.Trim(), PreSignedUrlLifetime));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var document = (await documents.GetByApplicationAsync(applicationId, cancellationToken))
            ...
            ?? throw new KeyNotFoundException(
                $"Document '{documentId}' was not found for application '{applicationId}'.");
