...
public sealed class ApplicationLifecycleAudit
{
    ...
    public static ApplicationLifecycleAudit Create(
        Guid applicationId,
        ApplicationStatus? fromStatus,
        ApplicationStatus toStatus,
        string actorReference,
        string? reason = null)
    {
        ...
        if (actorReference.Trim().Length > 200)
            throw new ArgumentException("Actor reference must be 200 characters or fewer.", nameof(actorReference));

        if (reason is not null && reason.Length > 1000)
            throw new ArgumentException("Lifecycle audit reason must be 1000 characters or fewer.", nameof(reason));

        return new ApplicationLifecycleAudit(
            Guid.NewGuid(),
            applicationId,
            fromStatus,
            toStatus,
