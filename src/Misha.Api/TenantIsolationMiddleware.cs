...
public sealed class TenantIsolationMiddleware(RequestDelegate next)
{
    ...
    private static bool TryGetGuid(RouteValueDictionary values, string key, out Guid id) { id = Guid.Empty; return values.TryGetValue(key, out var value) && Guid.TryParse(value?.ToString(), out id); }
}
