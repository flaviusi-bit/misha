...
public sealed class TenantIsolationMiddleware(RequestDelegate next)
{
    public async Task InvokeAsync(HttpContext context, MishaDbContext db)
    {
        ...
        if (IsCrossTenantAdmin(context.User))
        {
            await next(context);
            return;
        }
        ...
        if (TryGetGuid(context.Request.RouteValues, "id", out var id))
        {
            ...
            var allowed = path.StartsWith("/applicants/", StringComparison.OrdinalIgnoreCase)
                ? await db.Applications.AsNoTracking().AnyAsync(x => x.ApplicantId == id && x.TenantId == tenantId, context.RequestAborted)
    ...
    private static bool IsCrossTenantAdmin(ClaimsPrincipal user) =>
        user.FindAll("cognito:groups").Any(c => string.Equals(c.Value, "misha-admin", StringComparison.Ordinal));

    private static bool TryGetGuid(RouteValueDictionary values, string key, out Guid id) =>
        values.TryGetValue(key, out var value) && Guid.TryParse(value?.ToString(), out id);
}
