...
public static class ApplicantEndpoints
{
    public static void Map(WebApplication app)
    {
        ...
        app.MapPut("/applicants/{id:guid}/profile", async (
            Guid id, ApplicantProfileRequest request, HttpContext httpContext, ILoggerFactory loggerFactory,
            MishaDbContext db, CancellationToken ct) =>
        {
            ...
            try
            {
                ...
                applicant.SetProfile(new ApplicantProfile(request.FirstName, request.LastName, request.DateOfBirth,
                    request.Nationality, request.CountryOfBirth, request.PlaceOfBirth, request.Gender, request.Email, request.PhoneNumber));
                ...
                loggerFactory.CreateLogger("Misha.Security.Audit").LogInformation(
                    ...
                    applicant.Id, identity.Subject, identity.ClientId, applicant.ProfileCompleted);
        ...
        app.MapPost("/applications/{id:guid}/documents/presigned-upload", async (
            Guid id, PresignedUploadRequest request, DocumentService service, CancellationToken ct) =>
        {
            try
            {
        ...
        app.MapGet("/applications/{id:guid}/documents/{documentId:guid}/presigned-download", async (
            Guid id, Guid documentId, DocumentService service, CancellationToken ct) =>
        {
            try
            {
    ...
    private static ApplicantResponse ToResponse(Applicant applicant) =>
        new(applicant.Id, applicant.ExternalReference, applicant.FirstName, applicant.LastName, applicant.DateOfBirth,
            applicant.Nationality, applicant.CountryOfBirth, applicant.PlaceOfBirth, applicant.Gender, applicant.Email,
            applicant.PhoneNumber, applicant.ProfileCompleted, applicant.CreatedAtUtc, applicant.UpdatedAtUtc);
}

public sealed record ApplicantProfileRequest(
    string FirstName, string LastName, DateOnly DateOfBirth, string Nationality, string? CountryOfBirth,
    string? PlaceOfBirth, string? Gender, string? Email, string? PhoneNumber);

public sealed record ApplicantResponse(
    Guid Id, string ExternalReference, string? FirstName, string? LastName, DateOnly? DateOfBirth, string? Nationality,
    string? CountryOfBirth, string? PlaceOfBirth, string? Gender, string? Email, string? PhoneNumber, bool ProfileCompleted,
